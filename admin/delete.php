...
require('../db.php'); 
 ...
 {  
      $output = '';  
      $message = '';  
      $status = "select status from plot_detail where plot_id='".$_POST["item_id"]."'";
     $status_result = mysqli_query($connect, $status);   
     $status_row = mysqli_fetch_array($status_result);
     if($status_row["status"]=='Booked'){
             $query = "select * from plot_detail";  
           $message = 'Cannot Delete Plot It is in use'; 
     ...
     }else if($status_row["status"]=='Available'){
         
           $query = "delete from plot_detail WHERE plot_id='".$_POST["item_id"]."'";  
           $message = 'Parking Plot Deleted'; 
          $output .= '<div class="container"><div class="row">
                    <div class="col-12" align="right">
                        <button type="button" name="add" id="add" data-toggle="modal" data-target="#add_data_Modal" class="btn btn-primary add_btn" style="width:100%;margin:auto;margin-top:10px;"><span><i class="fas fa-car" style="margin-right:5px;"> </i>Add Parking</span> </button>
                    </div></div>
